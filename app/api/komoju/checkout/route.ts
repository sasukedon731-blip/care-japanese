...
const PRICE_TABLE: Record<Body["plan"], Record<30 | 90 | 180, number>> = {
  "3": { 30: 500, 90: 1200, 180: 2000 },
  "5": { 30: 500, 90: 1200, 180: 2000 },
  "7": { 30: 500, 90: 1200, 180: 2000 },
}

const AI_ADDON_PRICE: Record<30 | 90 | 180, number> = {
  30: 500,
  90: 1500,
  180: 3000,
}
...
export async function POST(req: Request) {
  try {
    ...
    const baseAmount = PRICE_TABLE[body.plan][body.durationDays]
    ...
    try {
      ...
      // 決済方法がテスト環境でまだ有効化されていない場合に備え、
      // payment_types を外してKOMOJU側の利用可能な決済一覧に任せる。
      if (e?.status === 400) {
