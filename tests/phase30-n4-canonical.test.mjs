...
test("Phase 30 N4 canonical dataset", () => {
  ...
  assert.equal(byId(95).question, "【休館】図書館は工事のため5月1日〜7日休みます。返却は入り口横のポストに入れてください。\n本を返したい人はどうしますか。")
  assert.equal(byId(95).choices[byId(95).correctIndex], "ポストに入れる")
  assert.notEqual(byId(95).question, byId(100).question)
  assert.equal(byId(100).sectionId, "reading")
  assert.equal(byId(100).correctIndex, 2)
  assert.equal(byId(100).choices[2], "今日の午後8時までにウェブサイトで申し込む")
